...
class coin:
    ...
    def collide(self, plr):
        r = pygame.Rect(
            self.x, self.y, self.img.get_rect()[2], self.img.get_rect()[3])
...
class camera:
    ...
    def update(self):
        ...
        if pr.colliderect(cr):
            ...
            if pr[0] > self.x:
                self.Or += plyr.speed * 4
            elif pr[0] < self.x:
                self.Or -= plyr.speed * 4
            if pr[1] > self.y:
                self.Ob += plyr.speed * 4
            elif pr[1] < self.y:
                self.Ob -= plyr.speed * 4
            return self.Or, self.Ob
    ...
    def update(self):
        ...
        if self.health <= 0:
            ...
            for y in range(random.randint(10, 20)):
                particles.append(particle(self.x, self.y, random.uniform(math.radians(0), math.radians(
...
def updatestars():
    ...
    c = 0
    if len(stars) < 20:
        stars.append(star(random.randint(outright, windw + outright),
                          random.randint(outbottom, windh + outbottom), 2, 2, (255, 255, 255), 0))
    for x in stars:
...
def updateenemies():
    ...
    for e in enemies:
        if e.update():
            ...
            dlt.append(c)
            for y in range(random.randint(10, 20)):
                particles.append(particle(e.x, e.y, random.uniform(0, math.radians(360)), pv, random.randint(
        ...
        if x not in dlt:
            ...
            if random.randint(1, 2) >= 1:
                ...
                for z in range(len(co)):
                    temp.append(
                        coin(enemies[x].x, enemies[x].y, random.randint(3, 6), co[z]))
                coins.append(temp)
    enemies = te
...
def updatemenus():
    ...
    for i in menus:
        if i.active:
            running, paused = i.update(tp, windw, windh, mouse)
        ...
        for e in enemies:
            ...
            if b.colliderect(a):
                ...
                if x.weapon == "EXPLOSIVE SQUARE":
                    ...
                    for y in range(random.randint(10, 20)):
                        particles.append(particle(x.x, x.y, random.uniform(0, math.radians(360)), pv, random.randint(
                            3, 4), cols[random.randint(0, len(cols) - 1)], random.randint(100, 200)))
                else:
                    for y in range(random.randint(1, 2)):
                        particles.append(particle(x.x, x.y, random.uniform(angle + math.radians(180), angle + math.radians(360)), pv, random.randint(
...
def drawplayer():
    ...
    pygame.draw.rect(screen, ((255, 0, 0)), ((
        (aimpos[0]) - outright, (aimpos[1]) - outbottom), (4, 4)), 1)
...
def drawstars():
    ...
    for x in stars:
        pygame.draw.rect(
            screen, x.colour, (x.x - outright + x.zx, x.y - outbottom + x.zy, x.w, x.h))
...
def drawproj():
    ...
    for i in projectiles:
        pygame.draw.rect(
            screen, (i.colour), ((i.x - outright, i.y - outbottom), (i.w, i.h)), 0)
...
def drawenemies():
    ...
    for e in enemies:
        pygame.draw.rect(
            screen, (e.colour), ((e.x - outright, e.y - outbottom), (e.w, e.h)), 3)
        hp = smallest.render(
            ("HP: " + str(math.ceil(e.health))), 5, (e.colour))
        t = hp.get_rect(
            center=(e.x + e.w / 2 - outright, e.y + e.h + 20 - outbottom))
        screen.blit(hp, (t[0], t[1]))
        if e.name != None:
            name = smallest.render(
                str(e.name), 5, (255, 255, 255), (80, 80, 80))
            t = name.get_rect(
...
scores = [[x.split("|")[0], x.split("|")[1]]
          for x in scorefile.read().split("\n")]
...
title = pygame.image.load("./assets/RectangleTitle.png").convert_alpha()
...
keys = [[K_a, False, pls * -1], [K_d, False, pls],
        [K_w, False, pls * -1], [K_s, False, pls]]
mouse = [[1, False]]
plyr = player(windw / 2, windh / 2, 20, 20, (random.randint(1, 255),
                                             random.randint(1, 255), random.randint(1, 255)), pls, 100)
camerarect = [[(windw / 2 - ((windw / 2) * pscreen)), (windh / 2 - ((windh / 2) * pscreen))],
              [((windw / 2) * pscreen) * 2, ((windh / 2) * pscreen) * 2]]
    ...
    if not paused and not score:
        ...
        if mouse[0][1] and tsl >= tbs:
            ...
            if weapons[cw] == "SINGLE SQUARE":
                tbs = 380
                dps = 8
                projectiles.append(bullet(aimpos[0] - 4 / 2, aimpos[1] - 4 / 2, 9, 9, dps, weapons[
                                   cw], (random.randint(1, 255), random.randint(1, 255), random.randint(1, 255)), angle, plsv))
            if weapons[cw] == "SPREAD SQUARE":
                tbs = 500
                dps = 3
                projectiles.append(bullet(aimpos[0] - 4 / 2, aimpos[1] - 4 / 2, 5, 5, dps, weapons[
                                   cw], (random.randint(1, 255), random.randint(1, 255), random.randint(1, 255)), angle, plsv))
                projectiles.append(bullet(aimpos[0] - 4 / 2, aimpos[1] - 4 / 2, 5, 5, dps, weapons[cw], (random.randint(
                    1, 255), random.randint(1, 255), random.randint(1, 255)), angle - math.radians(random.randint(6, 8)), plsv))
                projectiles.append(bullet(aimpos[0] - 4 / 2, aimpos[1] - 4 / 2, 5, 5, dps, weapons[cw], (random.randint(
                    1, 255), random.randint(1, 255), random.randint(1, 255)), angle + math.radians(random.randint(6, 8)), plsv))
            ...
            if weapons[cw] == "EXPLOSIVE SQUARE":
                tbs = 2000
                dps = 25.
                projectiles.append(bullet(aimpos[0] - 4 / 2, aimpos[1] - 4 / 2, 11, 11, dps, weapons[
                                   cw], (random.randint(1, 255), random.randint(1, 255), random.randint(1, 255)), angle, 0))
            tsl = 0.
        if not mainmenu:
            angle = plyr.angle(pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[
                               1], plyr.x + plyr.w / 2 - 4 / 2 - outright, plyr.y + plyr.h / 2 - 4 / 2 - outbottom)
        if plyr.alive:
            plyr.update()
            aimpos = plyr.aimerpos(
                plyr.x + plyr.w / 2 - 4 / 2, plyr.y + plyr.h / 2 - 4 / 2, angle)
        else:
            if not mainmenu:
                score = True
    if score:
        if random.randint(1, 100) == 2:  # Fireworks
            ...
            fx, fy = random.randint(
                outright, windw + outright), random.randint(outbottom, windh + outbottom)
            #
            midiout.set_instrument(118)
            #
            ...
            midiout.note_on(random.randint(35, 40), 127)
            #
            c = (random.randint(2, 255), random.randint(
                2, 255), random.randint(2, 255))
            #
                ...
                if clicked:
                    if windw != maxres[0]:
                        ...
                        screen = pygame.display.set_mode(
                            (windw, windh), pygame.FULLSCREEN, 32)
                    else:
                        full = False
                        windw, windh = 800, 600
                        screen = pygame.display.set_mode((windw, windh), 0, 32)
